...
from generate_thresholds_noelev import TFResLayer
...
class TFGeoPriorModel:
    ...
    def __init__(self, model_path, taxonomy):
        ...
        self.gpmodel.load_weights(model_path)
...
def main(args):
    ...
    im_df = im_df[["lat", "lng"]]
    dfh3 = im_df.h3.geo_to_h3(h3_resolution)
    dfh3 = dfh3.drop(
        columns=['lng', 'lat']
    ...
    eval_output = []
    np.random.shuffle(taxon_ids.values)
    for taxon_id in tqdm(taxon_ids):
        ...
        if taxon_range_recalls[taxon_range_recalls.taxon_id.eq(taxon_id)].shape[0] == 0:
            continue
        if taxon_range_recalls[(taxon_range_recalls['taxon_id'] == taxon_id) & (taxon_range_recalls['recall'] > 0.9)].empty:
            continue
        ...
        try:
            ...
            print("exception in processing range")
            gc.collect()
            continue
