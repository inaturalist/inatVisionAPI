...
class TestInatInferrer:
    def test_initialization(self, inatInferrer):
        ...
        tf.keras.models.load_model.assert_any_call(
            inatInferrer.config["vision_model_path"], compile=False
        )
        tf.keras.models.load_model.assert_any_call(
    ...
    def test_predictions_for_image(self, inatInferrer):
        test_image_path = os.path.realpath(
            os.path.dirname(__file__) + "/fixtures/lamprocapnos_spectabilis.jpeg"
        )
        results = inatInferrer.predictions_for_image(
            file_path=test_image_path, lat=42, lng=-71, filter_taxon=None, debug=True
        )
    ...
    def test_aggregate_results(self, inatInferrer):
        ...
        aggregated_scores = inatInferrer.aggregate_results(
            leaf_scores=combined_scores, debug=True
        )
    ...
    def test_h3_04_taxon_range_comparison(self, mocker, inatInferrer, taxon):
        inatInferrer.h3_04_geo_results_for_taxon = MagicMock(
            return_value={"aa": "0.1", "ab": "0.1"}
        )
        inatInferrer.h3_04_taxon_range = MagicMock(
            return_value={"ab": "0.1", "bb": "0.1"}
        )
        range_comparison_results = inatInferrer.h3_04_taxon_range_comparison(
            taxon["taxon_id"]
        )
        assert range_comparison_results == {"aa": 0, "ab": 0.5, "bb": 1}
